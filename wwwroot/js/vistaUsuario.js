$(document).ready(function () {
  ObtenerInformeCantidadesPersona();
  const filtrosBuscar = $(
...
async function ObtenerInformeCantidadesPersona() {
  ...
  const res = await authFetch("Actividades/CantidadesPersona", {
    method: "POST",
...
function MostrarInformeCantidad(data) {
  ...
  data.forEach((persona) => {
    ...
    filaPersona.innerHTML = `
      <td colspan="6" class="fw-bold categorias-agrupadas">
        ${persona.nombre}
      </td>
    `;
    ...
    filaTipo.innerHTML = `
      ...
      <th>TOTAL MINUTOS: ${tipo.total}min </th>
      <th>CANTIDAD REGISTROS: ${tipo.totalRegistros}</th>
    `;
